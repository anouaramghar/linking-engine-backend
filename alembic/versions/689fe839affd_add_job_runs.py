...
def upgrade() -> None:
    ...
    sa.Column('kind', sa.Enum('ingestion', 'analysis', 'publication', name='job_kind', native_enum=False, length=20), nullable=False),
    sa.Column('status', sa.Enum('queued', 'running', 'succeeded', 'failed', name='job_run_status', native_enum=False, length=20), server_default='queued', nullable=False),
    ...
    op.create_index('ix_job_runs_site_kind_status', 'job_runs', ['site_id', 'kind', 'status'], unique=False)
