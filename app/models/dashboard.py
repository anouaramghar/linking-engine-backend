...
class AgentActionReceipt(Base):
    """One human confirmation, spendable once by its originating MCP identity."""

    __tablename__ = "agent_action_receipts"

    id: Mapped[int] = mapped_column(primary_key=True)
    receipt_hash: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    principal_binding: Mapped[dict] = mapped_column(JSON)
    ...
    confirmed_by_user_id: Mapped[int | None] = mapped_column(
        ForeignKey("dashboard_users.id", ondelete="SET NULL"), index=True
    )
    # Durable human audit identity even if the dashboard account is removed.
    confirmed_by_telegram_id: Mapped[str] = mapped_column(String(32))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    expires_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), index=True)
    consumed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), index=True)
    execution_status: Mapped[str] = mapped_column(
        String(16), default="issued", server_default="issued", index=True
    )
