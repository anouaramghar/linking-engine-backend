...
Revision `d4e6f8a1b203` supersedes the feature branch's temporary global-Hybrid
revision without deleting it from Alembic history. Upgrading returns every
existing site and the database default to `standard`; explicit pilot enrollment
then happens through the controls below.
...
V1_PILOT_MAX_SOURCES_PER_RUN=50
...
Each run processes at most `V1_PILOT_MAX_SOURCES_PER_RUN` sources that still
have open slots. With the defaults, a run can create at most 150 suggestions.
Later runs continue deterministically with the next sources that have capacity,
so the bound controls editorial batch size without permanently excluding a
source.
